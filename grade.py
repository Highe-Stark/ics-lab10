...
class TimeoutException(LabErrorException):
    def __init__(self, hint, expect, binary=False):
        self.hint = hint
        if binary:
            self.expect, self.got = limited_binary_diff(expect, "")
        else:
            if expect == None:
                self.expect = ""
            else:
...
def start_proxy(listen_port):
    if strace:
        proxy = pexpect.spawn("bash -c 'strace -f -tt ./%s %d 2>>proxy-strace.log'" % (proxy_prog, listen_port), timeout=DEFAULT_TIMEOUT, logfile=open("proxy.log", "a"))
    else:
        proxy = pexpect.spawn("bash -c './%s %d 2>&1'" % (proxy_prog, listen_port), timeout=DEFAULT_TIMEOUT, logfile=open("proxy.log", "a"))
...
def expect_req(proc, req, timeout=-1):
    if proc != None:
        try:
            ...
            raise TimeoutException("Server wait for request line timeout", "%s %s HTTP/1.1" % (req.method, req.path))
        ...
        for header in req.headers:
            try:
                ...
                raise TimeoutException("Server wait for request header timeout", "%s: %s" % (header[0], header[1]))
            if line != "%s: %s\r\n" % (header[0], header[1]):
                raise MismatchException("Server got wrong header line", "%s: %s" % (header[0], header[1]), line)
        try:
            line = helper_readline(proc)
        except pexpect.exceptions.TIMEOUT:
            raise TimeoutException("Server wait for request header end line timeout", "")
        if line != "\r\n":
        ...
        if req.body != None:
            try:
                ...
                raise TimeoutException("Server wait for request body timeout", req.body, binary=True)
...
def expect_resp_header_only(proc, resp, timeout=-1):
    if proc != None:
        try:
            ...
            raise TimeoutException("Client wait for status line timeout", resp.head_line)
        if line != resp.head_line:
            raise MismatchException("Client got wrong response line", resp.head_line, line)
        for header in resp.headers:
            try:
                line = helper_readline(proc)
            except pexpect.exceptions.TIMEOUT:
                raise TimeoutException("Client wait for response header timeout", "%s: %s" % (header[0], header[1]))
            if line != "%s: %s\r\n" % (header[0], header[1]):
                raise MismatchException("Client got wrong header line", "%s: %s" % (header[0], header[1]), line)
        try:
            line = helper_readline(proc)
        except pexpect.exceptions.TIMEOUT:
            raise TimeoutException("Client wait for response header end line timeout", "")
        if line != "\r\n":
    ...
    if proc != None:
        ...
        if resp.body != None:
            try:
                ...
                raise TimeoutException("Client wait for response body timeout", resp.body, binary=True)
...
def main():
    ...
    parser.add_argument("--only", type=str, help="select one of tests")
    parser.add_argument("--outside", type=int, help="give a port so you can connect a proxy running outside")
    ...
    parser.add_argument("--proxy", type=str, help="the proxy program to use")
    ...
    if args.proxy == None:
        ...
        proxy_prog = args.proxy

    tests = candidate_tests[args.only]
